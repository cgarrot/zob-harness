...
import type { ZobLivePeerCard, ZobLivePeerStatus } from "./types.js";
...
export function ensureZpeerFields(repoRoot: string, peer: ZobLivePeerCard, roomId?: string, alias?: string): ZobLivePeerCard {
  const next: ZobLivePeerCard = {
    ...peer,
    projectId: buildZobComsProjectId(repoRoot),
    zpeerRoomId: safeZpeerRoomId(roomId ?? peer.zpeerRoomId) ?? DEFAULT_ROOM_ID,
    zpeerAlias: safeZpeerAlias(alias ?? peer.zpeerAlias) ?? generatedZpeerAlias(peer),
    zpeerLocalOnly: true,
    bodyStored: false,
  };
  return writeZobLivePeerCard(repoRoot, next);
}

export function refreshZpeerSelf(repoRoot: string, peer: ZobLivePeerCard, roomId?: string, alias?: string): ZobLivePeerCard {
  const ensured = ensureZpeerFields(repoRoot, peer, roomId, alias);
...
function peersInRoom(repoRoot: string, roomId: string): ZobLivePeerCard[] {
  ...
  return readZobLiveRegistrySnapshot(repoRoot).peers
    ...
    .filter((peer) => (peer.zpeerRoomId ?? DEFAULT_ROOM_ID) === roomId);
}

export function buildZpeerRoomSummary(repoRoot: string, self?: ZobLivePeerCard): ZpeerRoomSummary {
  const roomId = self?.zpeerRoomId ?? DEFAULT_ROOM_ID;
  const peers = peersInRoom(repoRoot, roomId);
  const counts: Record<ZobLivePeerStatus, number> = { online: 0, stale: 0, offline: 0 };
  const aliases = peers.map((peer) => peer.zpeerAlias ?? generatedZpeerAlias(peer)).sort();
  for (const peer of peers) counts[zpeerReachableStatus(peer)] += 1;
...
export function changeZpeerAlias(repoRoot: string, self: ZobLivePeerCard, requestedAlias: string): { ok: true; peer: ZobLivePeerCard } | { ok: false; reason: string } {
  const alias = safeZpeerAlias(requestedAlias);
  if (!alias) return { ok: false, reason: "alias must match [a-zA-Z][a-zA-Z0-9_-]{1,31}" };
  const roomId = self.zpeerRoomId ?? DEFAULT_ROOM_ID;
  const collision = peersInRoom(repoRoot, roomId).find((peer) => peer.sessionHash !== self.sessionHash && peer.zpeerAlias === alias && peer.status !== "offline");
  if (collision) return { ok: false, reason: `alias '${alias}' is already used in room '${roomId}'` };
  return { ok: true, peer: refreshZpeerSelf(repoRoot, self, roomId, alias) };
}
...
export function changeZpeerRoom(repoRoot: string, self: ZobLivePeerCard, requestedRoom: string): { ok: true; peer: ZobLivePeerCard } | { ok: false; reason: string } {
  ...
  const alias = self.zpeerAlias ?? generatedZpeerAlias(self);
  const collision = peersInRoom(repoRoot, roomId).find((peer) => peer.sessionHash !== self.sessionHash && peer.zpeerAlias === alias && peer.status !== "offline");
  if (collision) return { ok: false, reason: `alias '${alias}' already exists in target room '${roomId}'; rename first` };
  return { ok: true, peer: refreshZpeerSelf(repoRoot, self, roomId, alias) };
}
...
export async function sendZpeerPrompt(repoRoot: string, self: ZobLivePeerCard, targetAliasInput: string, transientPrompt: string, awaitReply: (msgId: string) => Promise<{ status: string; envelope?: { outputHash?: string; transientResponse?: string; errorHash?: string } }>, options: ZpeerSendOptions = {}): Promise<ZpeerSendResult> {
  const roomId = self.zpeerRoomId ?? DEFAULT_ROOM_ID;
  const senderAlias = self.zpeerAlias ?? generatedZpeerAlias(self);
  ...
  const candidates = peersInRoom(repoRoot, roomId).filter((peer) => peer.zpeerAlias === targetAlias && peer.sessionHash !== self.sessionHash);
  if (targetAlias === self.zpeerAlias) return finish("attempt", { status: "blocked", reason: "cannot send to self", targetAlias, taskHash, bodyStored: false });
  ...
  const targetReachableStatus = zpeerReachableStatus(target);
  ...
  const topologyBlocker = validateZpeerTopology(repoRoot, self, target);
  if (topologyBlocker) return finish("attempt", { status: "blocked", reason: topologyBlocker, targetAlias, taskHash, bodyStored: false }, 1);
  if (target.transport !== "local_socket" || target.endpoint.startsWith("pending-") || target.endpoint === "observe-only") return finish("attempt", { status: "blocked", reason: `peer @${targetAlias} is not reachable by local_socket`, targetAlias, taskHash, bodyStored: false }, 1);
  ...
  const msgId = `zpeer:${self.sessionHash.slice(0, 8)}:${target.sessionHash.slice(0, 8)}:${Date.now()}`;
  ...
  const envelope = buildZobLiveEnvelope({
    ...
    runId: `zpeer:${roomId}`,
    sender: senderAlias,
    receiver: target.zpeerAlias ?? target.roleId,
    team: self.team,
    taskHash,
  ...
  try {
    const ack = await sendZobLocalEnvelope(target.endpoint, envelope, { timeoutMs: 5_000 });
    ...
    emitFeedback("delivered", { status: "delivered", msgId, targetAlias, taskHash, bodyStored: false });
    emitFeedback("waiting", { status: "waiting", reason: mode === "long" ? "waiting for long peer reply" : "waiting for peer reply", msgId, targetAlias, taskHash, bodyStored: false });
    const reply = await awaitReply(msgId);
    if (reply.status === "completed") {
      const transientResponse = reply.envelope?.transientResponse;
      const result = finish("terminal", { status: "reply", msgId, targetAlias, taskHash, outputHash: reply.envelope?.outputHash ?? (transientResponse ? sha256(transientResponse) : undefined), transientResponse, bodyStored: false }, 1);
    ...
    const result = finish("terminal", { status: "error", reason: reply.envelope?.errorHash ?? "peer response error", msgId, targetAlias, taskHash, bodyStored: false }, 1);
