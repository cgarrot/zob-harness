...
| worklist / liveness | Append-only metadata-only blackboard: append events, read derived directives, lease/satisfy by content hash, deliver idempotent DIRECTIVE_READY, run the liveness watchdog (observe/escalate), build/query the dependency DAG | `zob_worklist` (append/directives/claim/satisfy/deliver/validate/observe/escalate/dag); skill `zob-worklist` |
