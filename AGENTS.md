...
- ZTeam/ZAgent teams: `/zteam`, `/zagent`, `zob_zteam_hot_add`, `zob_zteam_remove`, tmux-backed agent teams, ZPeer rooms/aliases/membership -> `.pi/skills/zob-zagent-creator/SKILL.md`, `.pi/skills/zob-coms-v2-live/SKILL.md`, and `.pi/skills/zob-coms-safety/SKILL.md`; use the existing ZTeam/ZAgent workflow instead of building ad hoc tmux or file-room communication. If a suitable command/tool exists, use it or produce its exact plan; do not recreate it by hand.
...
- Worklist blackboard: `zob_worklist` (append/directives/claim/satisfy/deliver/validate/observe/escalate/dag) -> `.pi/skills/zob-worklist/SKILL.md`; append-only metadata-only and hash-only/body-free; observe is true only when no directive is open; never stores raw bodies and never enables network.
- Context: `zob_context_search`, `zob_context_*` -> registry no-ship notes plus `.pi/skills/zob-context-discovery/SKILL.md`, `.pi/skills/zob-harness/SKILL.md`, and `.pi/skills/zob-spec/SKILL.md` as applicable; for exploratory/natural-language repo discovery, start with the active search backend (`zob_context_search`/ColGREP when ready, grep/find/read fallback) before broad grep/find, and reuse it at context pivot points (new subsystem/domain, ambiguous file area, `fallback_status` suggesting narrower paths, repeated low-signal grep/find, unfamiliar code before edits, or unknown validation/test failure); if the native tool is not exposed but bash is available, run the compact wrapper `npm run --silent zob:context:query -- --query "<query>" --max-results 6 --max-context-lines 1` before `rg`/`grep`; then verify exact claims with grep/read/file refs and use grep/read directly when exact identifiers/paths are already known; never broad-grep `.pi` without pruning `.pi/sessions` and `.pi/agent-sessions`.
...
- `/zagent` / `/zteam` / `zob_zteam_hot_add` / `zob_zteam_remove` — governed full-session ZAgent/ZTeam creation, hot-add/remove, launch-plan, and ZPeer room membership; use these for tmux-backed teams instead of custom file-room scaffolds
