...
requireIncludes('.pi/extensions/zob-harness/src/runtime/commands.ts', [
  ...
  'reset <id> [--dry-run]',
  'reset-plan ${id}',
  'zob.zteam-reset-command.v1',
  'zteam_${action}_dry_run',
  'zteam_${action}_execute_blocked',
  'zteam_${action}_execute',
  ...
  'execute blocked: launcher does not expose non-attaching new reset action',
  ...
  'actions: status -> new (/new in each existing team tmux window)',
  'spawnSync("bash", [plan.launcherPath, plan.resetAction]',
  'dry-run only: no tmux and no /new sent',
...
requireIncludes('.pi/skills/zob-zagent-creator/SKILL.md', [
  ...
  '`/zteam reset` sends Pi `/new` to existing scoped tmux agent windows without closing tmux or cleaning leases directly',
